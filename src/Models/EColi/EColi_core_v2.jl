...
function load_ecoli_dataframes(;model_data_dir = Chemostat.ECOLI_CORE_V2_MODEL_DATA_DIR,
                        file_prefix = Chemostat.ECOLI_MODELS_DATA_FILES_PREFIX)


    sto = CSV.read(joinpath(model_data_dir, file_prefix * "_s.csv"), allowmissing=:none, types=[Int, Int, Float64, String, String]);
    mets = CSV.read(joinpath(model_data_dir, file_prefix * "_mets.csv"), allowmissing=:none, types=[String, Float64, Float64, Float64, Float64, Float64]);
    rxns = CSV.read(joinpath(model_data_dir, file_prefix * "_rxns.csv"), allowmissing=:none, types=[String, Float64, Float64, Float64, Float64]);
